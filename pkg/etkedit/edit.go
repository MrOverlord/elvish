		...
		etk.WithInit(
			...
			"binding", func(ev term.Event, c etk.Context, r etk.React) etk.Reaction {
				...
				if reaction == etk.Unused {
					handled := ed.callBinding(&ed.globalBinding, ev)
					if handled {
						return etk.Consumed
					} else if k, ok := ev.(term.KeyEvent); ok {
