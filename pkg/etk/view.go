...
func (v EmptyView) Render(width, height int) *term.Buffer { return new(term.Buffer) }
...
func (v BoxView) Render(width, height int) *term.Buffer {
	...
	if v.Horizontal {
		budget = width
		render = func(v View, budget int) *term.Buffer {
			buf := v.Render(budget, height)
	...
	for i, child := range v.Children {
		if child.Flex {
			...
			childBufs[i] = render(child.View, budget)
		}
	}
	for i, child := range v.Children {
		if child.Flex {
			childBufs[i] = render(child.View, budget/flexChildren)
			flexChildren--
		}
	}
